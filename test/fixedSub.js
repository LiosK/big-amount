...
describe("#fixedSub()", () => {
  it("returns the same fraction as `#sub()` + `#quantize()`", () => {
    ...
    runTestOnPairs((xn, xd) => {
      ...
      runTestOnPairs((yn, yd) => {
        ...
        const expected = left.sub(right).quantize(left.den);
        const actual = left.fixedSub(right);
  ...
  it("rounds the numerator as expected (test [-5.0, 5.0])", () => {
    ...
    const test = (num, oldDen, newDen, expected) => {
      assert.strictEqual(
        new BigAmount(0n, newDen).fixedSub(new BigAmount(-num, oldDen)).num,
        expected
      );
    };
    ...
    const testWithMode = (num, oldDen, newDen, mode, expected) => {
      assert.strictEqual(
        new BigAmount(0n, newDen).fixedSub(new BigAmount(-num, oldDen), mode)
          .num,
        expected
      );
    };
